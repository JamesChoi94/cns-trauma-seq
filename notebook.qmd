...
I double checked to see whether specific input/output files are needed for downstream RNA velocity analysis, specifically scVelo. All that is needed is the resulting `.bam` file, so I am running `cellranger count` with nearly default settings.


## 2022-10-18

`cellranger count` is done for all samples. No obvious errors were reported other than run-time errors (I set the pegasus job wall time to 10 hours, which was not enough). To get through processing faster, I submitted a separate pegasus `big-mem` job for each time point (4 jobs total). I submitted the job on Tuesday 2022-10-18 at 6:30am and most jobs were finished by same day 2:30pm. All in all, not a slow process.


```{bash}
cd /mnt/d/cns-trauma-seq/data/cellranger
rsync -avn \
  --exclude="_*" \
  --exclude="**/SC_RNA_COUNTER_CS" \
  --exclude="**/outs/analysis/*" \
  --exclude="**/outs/*.bam" \
  --exclude="**/outs/*.bam.bai" \
  --include="**/outs/*.cloupe" \
  --include="**/outs/*.cloupe" \
  --include="**/outs/*.h5" \
  --include="**/outs/*.html" \
  "jsc228@pegasus.ccs.miami.edu:/scratch/projects/lemmon/jsc228/cns-trauma-seq/data/cellranger" "./"
```
